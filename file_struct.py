        ...
        if "[Content_Types].xml" in dir:
            for file in dir:
                if file == "word/document.xml":
                    if ex == ".docx":
                        print("[*]{}:文件类型为Microsoft Office Word".format(sfile))
                    else:
...
def filetype(filepath,filesize):
    ...
    if ftype == "ZIP":
        ...
        if ex in tl[f_hcode][1]:
            print("[*]%s,文件大小%.3f KB" % (sfile, filesize))
            print("[*]{},文件类型为{}" .format(sfile,ftype))
            pass
        else:
            filelist.append(filepath)
            print("[!]%s,文件大小%.3f KB" % (sfile, filesize))
            print("[!]文件后缀被篡改，文件类型为%s" % ftype)
            if not ftype.endswith(("V2","V4","Word/Excel")):
                f_ext=tl[hcode][1][0]
                path,_=os.path.splitext(sfile)
                new_path=path+f_ext
                os.rename(sfile,new_path)
